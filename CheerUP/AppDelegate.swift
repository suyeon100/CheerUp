...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func scheduleDailyNotification(message: String, hour: Int, minute: Int) {
        ...
        let request = UNNotificationRequest(identifier: "dailyMessage", content: content, trigger: trigger)

        UNUserNotificationCenter.current().add(request) { error in
            if let error = error {
                print("알림 설정 오류: \(error.localizedDescription)")
            } else {
                print("알림이 예약되었습니다.")
            }
        }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "CheerUP")
